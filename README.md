...
- ファイル操作（新規フォルダ／新規ファイル／リネーム／削除（ごみ箱・完全）／
  コピー・切り取り・貼り付け／パス・名前のコピー）。`Ctrl+Z` で名前の変更・新規作成・
  移動・コピーを元に戻せる（削除の取り消しは未対応 ─ ごみ箱から復元する）
- 単一インスタンス。フォルダを渡して起動すると、動いている Kite の新しいタブとして
  開き前面に出る。別の窓が欲しいときは `Ctrl+N`（`--new-window`）
...
- タブとセッションが増えたらバーが折り返す。右端で切り捨てて押せなくなることはない
- 日本語 IME の変換候補ウィンドウをキャレット位置に追従
- ウィンドウ位置・セッション・ブックマークの永続化

未実装（検索・サムネイル・仮想フォルダなど）と今後の計画は
**[docs/ROADMAP.md](docs/ROADMAP.md)**。直近の最重要項目は「PC」「ごみ箱」などの
仮想フォルダ、検索。
...
| ショートカット一覧 | `F1` |
| 設定画面 | `Ctrl+,`（キー割り当ては `Ctrl+F1`、設定フォルダを開くのは `Ctrl+Alt+,`） |
