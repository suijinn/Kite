...
- サイドバー（クイックアクセス／ブックマーク／ドライブ + 空き容量バー）。区画ごとに
  ...
  どの行もシェルの実アイコン（オーバーレイ込み）で出るので、同期状態や作業コピーの
  変更が並びのまま読める
...
- 名前はその場で編集する。リネームは対象の行、新規作成はカーソルの 1 行下に借りた行、
  セッション名はそのチップの中 ─ どれも「どれの名前を訊かれているか」が位置で分かる。
  リネームは拡張子を除いた部分が選択された状態で開くので、打てばそのまま置き換わる
...
| 新しいセッション | `Ctrl+Alt+N` |
| セッション名を変更 | `Ctrl+Alt+R`（セッションチップのダブルクリックも可） |
| 新しいウィンドウ | `Ctrl+N` |
| 元に戻す | `Ctrl+Z` |
