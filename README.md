...
- 選択（範囲・トグル・全選択・反転）、マウス操作一式。`Space` の選択はカーソル移動で
  消えないので、離れた項目をいくつでも拾える
- マウスが乗っている項目のハイライト（一覧の行、サイドバー、タブと閉じるボタン、
  タブ・セッションの `+`、パンくず）。何かを掴んでいる間は消える ─ ドラッグ中に
  通りすがった行まで光ると、掴んでいるものの行き先が読めなくなる
- 一覧先頭の `..` 行、空きスペースのダブルクリックで親フォルダへ
- コンテキストメニューは `Shift` で拡張（エクスプローラーと同じ作法）。既定で拡張を
  出さないので、working copy の中で「Git クローン」のような**普段は隠されている動詞**
  を勧められることがない。表示中のフォルダに対しては、エクスプローラーが一覧の余白で
  出すのと同じ「背景」メニュー（新規作成やターミナルが並ぶ側）
- フォーカスの見える化: キーボードのいるペインを彩度を抑えた青の枠で示し、
  他のペインは一段落とす。**ウィンドウ自体が非アクティブなら青は消えて無彩色に戻る**
- 版とコミットの表示（タイトルバー・セッションバー右端・`F1` の見出し、
  および両 exe のプロパティ）
