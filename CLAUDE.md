...
- **フォルダのメニューは 2 種類あり、取り違えると別の献立が出る。** 項目のメニュー
  ...
  一覧で右クリックしたときのもので、TortoiseGit の「Git クローン」（＝このフォルダへ
  クローンする）はここに出る。表示中のフォルダに対して欲しいのは背景のメニュー
  （`IShellFolder::CreateViewObject`）で、エクスプローラーが一覧の余白で出すもの。
  要求の `background` がこれを切り替える。**選択が空のときと `Cmd::FolderContextMenu`
  は必ず背景側**。背景側では `InvokeCommand` に `lpDirectory` も渡す（「新規作成」の
  行き先になる）。
