...
/// @brief 既定のファイルマネージャーとしての登録状態。
///
/// 「登録されていない」と「別の Kite が登録されている」を分けてあるのは、後者だけが
/// **利用者に伝える価値のある事故**だから ─ exe を移した（ポータブル運用で zip を
/// 展開し直した）時点で登録は死んでいるのに、画面はそれを言う機会を持たない。
/// **他のアプリが登録されている場合は `No`。** そちらは事故ではなく利用者の選択で、
/// 起動のたびに「Kite は既定ではありません」と言うのはただの押し売りになる。
enum class DefaultManager : uint8_t {
    Unsupported,  ///< この OS には登録の口が無い
    No,           ///< 登録されていない（他のアプリが登録されている場合を含む）
    Yes,          ///< この exe が登録されている
    Other,        ///< 同じ名前の exe が別の場所から登録されている
};
...
public:
    ...
    /// @brief 既定のファイルマネージャーとして登録されているかを返す。
    /// @return 登録状態
    /// @note 起動のたびに 1 回だけ呼ぶ想定。答えを出すのはレジストリの読み取り
    ///       数回なので、毎フレーム引ける値ではないが起動を遅らせもしない
    virtual DefaultManager DefaultManagerState() = 0;

    /// @brief 既定のファイルマネージャーとしての登録を付け外しする。
    /// @param[in] on true で登録、false で解除
    /// @return 書き換えられたら true
    /// @note **昇格を求めてはならない。** 書き込むのは `HKCU` だけ ─ ファイラーを
    ///       1 つ入れるために管理者権限を要求するのは、そのファイラーが何をするか
    ///       まだ知らない人にとって最悪の第一印象になる
    /// @note **解除は元に戻すこと。** 付けるときに «その値が元から在ったか» を控えて
    ///       おき、外すときにその形へ戻す ─ 戻せなければ、Kite を消したあとフォルダが
    ///       1 つも開かない Windows が残る
    virtual bool SetDefaultManager(bool on) = 0;
};
