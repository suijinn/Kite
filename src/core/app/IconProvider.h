...
public:
    ...
    /// @brief 取得済みの対応をすべて捨て、次の描画で取り直させる。
    /// @note オーバーレイはファイルの状態を映すので、コミットや同期でファイルが
    ///       変わればアイコンも変わる。シェルからの通知は受けていないため、
    ///       利用者の操作で一覧を取り直すときに合わせて捨てる
    /// @note UI スレッドからのみ呼ぶこと
...
public:
    ...
    ///         Invalidate() 後は、取り直しが済むまで直前の識別子を返し続ける
    ...
    /// @brief 取得済みの結果だけを捨て、次の Lookup() で取り直させる。
    /// @note Clear() と違い、取得中のパスはそのまま待たせる ─ 途中で捨てると
    ///       同じパスを二重に頼むことになる。届いた結果は Store() が捨てる
    void Invalidate();

    /// @brief 覚えている識別子を無効なものとして捨て、取り直させる。
    /// @note Invalidate() との違いは識別子まで 0 に戻すこと。取得元が入れ替わって
    ///       識別子の意味が変わった場合に使う ─ 描き続けると別の絵が出るので、
    ///       取り直しが済むまではベクタ描画に戻すしかない
    /// @note 取得中のパスは取得中のまま残す。今まさに届こうとしている結果は
    ///       新しい取得元のものなので、捨てずに Store() で受け取る
    void Forget();
