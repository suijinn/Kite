...
///
/// **3 つの «位置» であって、«縦置き» と «統合» の 2 つの真偽値ではない。** 独立した
/// 切り替えにすると `(位置 = 上, 統合 = on)` という意味を持たない組み合わせが作れて
/// しまい、`settings.ini` にもそのまま残る ─ 取りうる状態がそのまま画面の答えに
/// なるほうが、取り違えようが無い（「列 = 並べ替えの基準」と同じ決め方）。
enum class TabBarPosition : uint8_t {
    Top,   ///< 一覧の上。横に並べ、収まらなければ行を増やす
    Left,  ///< 一覧の左。縦に積み、収まらなければスクロールする
    /// サイドバーの中。上端の «タブ» 区画に入り、帯 1 本ぶんの幅が一覧へ戻る。
    /// **入れるのはタブバーが 1 本しか要らないときだけ**（App::tabsInSidebar）─
    /// サイドバーはウィンドウに 1 つしかないので、ペインが 2 つあるとどちらの
    /// タブなのかを言えない。条件を欠いたら Left として立つ。
    Sidebar,
};
