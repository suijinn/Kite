...
const DefaultBinding kDefaults[] = {
    ...
    // 行き先の窓口（ブックマーク＋開いているタブ＋ドライブ）。**移動はファイラーの
    // 主動詞**なので、コマンドパレット（Ctrl+Shift+P）より打ちやすい席をこちらに ─ VS Code が
    // Ctrl+P を «場所»、Ctrl+Shift+P を «コマンド» に割り当てて短いほうを場所に与えて
    // いるのと同じ判断（ROADMAP P3-12）。番号（Alt+Shift+<桁>）が尽きた先へ届く唯一の
    // 道でもある。
    // **Alt+B から移した。** 既存の keys.ini には（旧名の）bookmark.list の行が書かれて
    // いるので、和音が移るのは新規の環境だけ（Ctrl+N と同じ罠 ─ 案内するときはそれを
    // 踏まえて）。
