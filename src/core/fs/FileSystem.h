...
public:
    ...
    /// @brief ドライブ文字を持たないマウント先を列挙する。
    /// @return マウント先の一覧。ドライブ直下（`Roots()` の担当）は含まない
    /// @note **サイドバーには出さない。** これは «そのパスがどのボリュームに
    ///       載っているか» を答えるための一覧で、`C:\\Box` のようにフォルダへ
    ///       載ったクラウドを «C: は固定ディスク» の一言で片付けないためにある
    /// @note 種別の読み方は `Roots()` と同じでなければならない ─ 2 通りあると、
    ///       同じ 1 つのボリュームがどちらから引いたかで違う答えを持つ
    virtual std::vector<Root> MountPoints() = 0;
